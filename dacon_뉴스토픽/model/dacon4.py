...
x_train = pad_sequences(x_train, padding='pre', maxlen=10)
x_test = pad_sequences(x_test, padding='pre', maxlen=10)
...
# print(x_train.shape, x_test.shape) # (36523, 10) (9131, 10)
# print(x_train)
...
model = Sequential()
                 # 단어사전의 개수                   단어수, 길이
model.add(Embedding(input_dim=15000, output_dim=128, input_length=10))
...
model.fit(x_train, y_train, epochs=300, batch_size=256, validation_split=0.01, callbacks=[es, mcp])
end_time = time.time() - start_time

# model = load_model('./_save/dacon0728_1753_.0001-1.0666.hdf5')
...
7차 / mcp / dacon0728_1716_.0002-0.6706
acc :  0.7888511419296265

8차 / es 수정 / dacon0728_1747_.0002-0.6889
acc :  0.7830467820167542
